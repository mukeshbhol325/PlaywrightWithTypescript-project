...
test.only('practice frames',async ({page})=>{

await page.goto("https://ui.vision/demo/webtest/frames/");
console.log(page.frames().length);
let frame1=await page.frameLocator("[src='frame_2.html']");
//await frame1.locator("[name='mytext2']").fill("value");
const frame2=page.frame({url:"https://ui.vision/demo/webtest/frames/frame_2"});
if(frame2){

    frame2.locator("[name='mytext2']").fill("value");
}
await page.waitForTimeout(5000);
})
